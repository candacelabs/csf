...
- `candace/candaceos/` in the private `candace-server` monorepo, which is
  canonical; and
- `candaceos/` in `candacelabs/csf`, the generated public repository that
  is a one-way snapshot of the whole `candace/` export root.
...
## If this checkout is candacelabs/csf
...
The snapshot is the whole `candace/` tree. Each publish of `candacelabs/csf`
...
- **Every edit is a public release.** This folder sits inside an active
  export root: the moment a change reaches `main`, the whole `candace/` tree is
  published to `candacelabs/csf` and tagged. Run `candace export validate`
...
## What cannot be done from candacelabs/csf alone
